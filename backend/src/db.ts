...
// 加载本地化翻译
function loadLocalTranslations(): Map<string, string> {
  const translationMap = new Map<string, string>();
  try {
    const csvPath = path.join(__dirname, '../../list.csv');
    if (!fs.existsSync(csvPath)) return translationMap;
...
function syncDatabase() {
  ...
  try {
    const templatePath = path.join(__dirname, '../../main_storage_itemscsv.txt');
